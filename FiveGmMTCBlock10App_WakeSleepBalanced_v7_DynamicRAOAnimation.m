classdef FiveGmMTCBlock10App_WakeSleepBalanced_v7_DynamicRAOAnimation < handle
    % FiveGmMTCBlock10App
    % ---------------------------------------------------------------------
    ...
    %        app = FiveGmMTCBlock10App_WakeSleepBalanced_v7_DynamicRAOAnimation;
    %
    % Main features:
    ...
    methods
        function app = FiveGmMTCBlock10App_WakeSleepBalanced_v7_DynamicRAOAnimation()
    ...
    methods (Access = private)
        ...
        function createParameterTab(app)
            ...
            grid = uigridlayout(app.ParamTab, [24 2]);
            grid.RowHeight = repmat({28}, 1, 24);
        ...
        function runSimulation(app)
            ...
            % Among awake devices, only a subset actually generates a packet
            % and transmits in the current RAO/slot.
            pTxGivenAwake = min(1, max(0, p.TxProbabilityGivenAwake));
            active = awake & (rand(N, 1) < pTxGivenAwake);

            % For teaching/demo: if devices are awake and p_tx|awake > 0,
            % keep at least one transmitting device so the diagram has a
            % visible uplink path.
            if any(awake) && ~any(active) && pTxGivenAwake > 0
                awakeIdxTmp = find(awake);
            ...
            % 8. Delay model
            T_access = 0.010 + retryCount .* 0.020;
            ...
            ID = (1:N).';
            app.Devices = table(ID, type, x, y, z, d_m, awake, active, preamble, ...
                collision, PL_dB, Pr_dBm, SNR_dB, SINR_dB, ...
                InterferencePower_dBm, potentialLinkOK, actualRadioOK, ...
                linkOK, success, retryCount, delay_s, E_J, cumulativeEnergy_J, ...
                ...
                'Distance_m','Awake','Active','Preamble','Collision', ...
                'PathLoss_dB','ReceivedPower_dBm','SNR_dB','SINR_dB', ...
                'InterferencePower_dBm','PotentialLinkOK','ActualRadioOK', ...
                'LinkOK','Success','RetryCount','Delay_s','Energy_J', ...
        ...
        function updateDashboard(app)
            ...
            lines = { ...
                ...
                'VISUAL LEGEND'; ...
                '------------------------------------------------------------'; ...
                'Gray: sleep | Blue: awake idle | Green: TX success | Red: TX collision | Yellow: TX link failed'; ...
